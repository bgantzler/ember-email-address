...
import {collection, create, isVisible, triggerable} from 'ember-cli-page-object';
import {clickTrigger, selectChoose as powerSelectChoose} from 'ember-power-select/test-support/helpers';
...
module('Integration | Component | select-multiple-emails', function(hooks) {
  ...
  test('Component excepts a list of pre-selected emails', async function(assert) {
    ...
    assert.equal(page.selectEmails.selected.length, 1, "One pre-selected email");
  });
  ...
  test('Sends onChange to parent', async function(assert) {
    ...
    this.set('onChangeSelectedSpy', (selected) => {
      assert.equal(selected[0], 'bar@foo2.com', "Called onChange with selected email option");
    })
    ...
    await render(hbs`<EmberEmailAddresses@EmailAddresses
                      ...
                      />`);
    await clickTrigger(page.selectEmails.scope);
    await powerSelectChoose(page.selectEmails.scope, '.ember-power-select-option', 1);
  });
